...
<div x-show="showComments" x-transition>
    ...
    <div class="bg-white p-6 rounded-xl shadow-lg mb-8 border border-gray-200">
        <form wire:submit.prevent="addMainComment">
            ...
            @auth
                <p class="text-sm text-gray-600 mb-2">
                    Anda berkomentar sebagai: <span class="font-bold">{{ auth()->user()->name }}</span>
                </p>
            @else
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        <input type="text" wire:model.defer="mainComment.guest_name"
                               placeholder="Nama Anda*"
                               class="w-full border-gray-300 rounded-md shadow-sm px-3 py-2 focus:border-gizila-dark focus:ring-gizila-dark/50">
                        @error('mainComment.guest_name')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
                        @enderror
                    </div>
                    <div>
                        <input type="email" wire:model.defer="mainComment.guest_email"
                               placeholder="Email Anda*"
                               class="w-full border-gray-300 rounded-md shadow-sm px-3 py-2 focus:border-gizila-dark focus:ring-gizila-dark/50">
                        @error('mainComment.guest_email')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
                        @enderror
                    </div>
                </div>
            ...
            <textarea wire:model.defer="mainComment.body" rows="4"
                      class="w-full border-gray-300 rounded-md shadow-sm px-3 py-2 focus:border-gizila-dark focus:ring-gizila-dark/50"
                      placeholder="Tulis tanggapan Anda di sini..."></textarea>
            @error('mainComment.body')
                <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
            @enderror

            <div class="hidden">
                <input type="text" wire:model="mainComment.honeypot">
            </div>

            <div class="text-right mt-4">
                <button type="submit"
                        class="bg-gizila-dark text-white font-semibold py-2 px-6 rounded-lg hover:bg-opacity-90 transition-colors duration-300">
                    Kirim Tanggapan
                </button>
            </div>
        </form>
    ...
    <div class="space-y-6">
        @forelse ($comments as $comment)
            <div wire:key="comment-{{ $comment->id }}" 
                 class="bg-white p-4 sm:p-6 rounded-xl shadow-lg border border-gray-200 {{ $replyingTo === $comment->id ? 'ring-2 ring-gizila-dark' : '' }}">
                
                {{-- Komentar Utama --}}
                <x-comment-card :comment="$comment" :replying-to="$replyingTo" />

                {{-- Tombol Balas & Hapus --}}
                <div class="flex items-center space-x-4 mt-3 ml-12 md:ml-14">
                    <button wire:click.prevent="startReply({{ $comment->id }})" 
                            class="text-sm font-semibold text-gizila-dark hover:underline">
                        Balas
                    </button>
                ...
                {{-- Form Balasan kalau komentar ini yang dipilih --}}
                        ...
                        <form wire:submit.prevent="addReply">
                            @guest
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                    <div>
                                        <input type="text"
                                               wire:model.defer="replyComment.guest_name"
                                               placeholder="Nama Anda*"
                                    ...
                                    <div>
                                        <input type="email"
                                               wire:model.defer="replyComment.guest_email"
                                               placeholder="Email Anda*"
                                               class="w-full text-sm border-gray-300 rounded-md shadow-sm px-3 py-2 focus:border-gizila-dark focus:ring-gizila-dark/50">
                ...
                @if ($comment->replies->count() > 0)
                    <div class="mt-4 ml-8 md:ml-14 pl-4 border-l-2 border-gray-200 space-y-4">
                        @foreach ($comment->replies as $reply)
                                                <div wire:key="reply-{{ $reply->id }}" class="flex items-start justify-between">
                        <div class="flex-1">
                            <x-comment-card :comment="$reply" :replying-to="$replyingTo" />
                        </div>
                        <div class="ml-4">
                            <button wire:click.prevent="deleteComment({{ $reply->id }})"
                                    class="text-sm font-semibold text-red-600 hover:underline">
                                Hapus
                            </button>
                        </div>
